...
class PHPMailer_mymail
{
    ...
    protected function addAnAddress($kind, $address, $name = '')
    {
        if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
            ...
            if ($this->exceptions) {
                throw new phpmailerException_mymail('Invalid recipient array: ' . $kind);
            }
        ...
        if (!$this->validateAddress($address)) {
            ...
            if ($this->exceptions) {
                throw new phpmailerException_mymail($this->lang('invalid_address') . ': ' . $address);
            }
    ...
    public function send()
    {
        ...
        } catch (phpmailerException_mymail $e) {
            $this->mailHeader = '';
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (count($bad_rcpt) > 0) { //Create error message for any bad addresses
            throw new phpmailerException_mymail($this->lang('recipients_failed') . implode(', ', $bad_rcpt));
        }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new phpmailerException_mymail($this->lang('signing') . ' OpenSSL extension missing.');
                }
    ...
    public function set($name, $value = '')
    {
        try {
            if (isset($this->$name)) {
                ...
                throw new phpmailerException_mymail($this->lang('variable_set') . $name, self::STOP_CRITICAL);
            }
        } catch (Exception $e) {
            $this->setError($e->getMessage());
