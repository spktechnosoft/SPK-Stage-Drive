...
class mymail_actions {
	...
	private function add_subscriber_action($args, $explicit = true) {
		...
		if(mymail_option('do_not_track') && isset($_SERVER['HTTP_DNT']) && $_SERVER['HTTP_DNT'] == 1) return;
	...
	public function get_dashboard_actions($scale = 'days', $limit = 7, $offset = 0, $sets = NULL){
		...
		if(is_null($sets)) $sets = array('opens','clicks','unsubscribes','bounces');
	...
	public function get_campaign_actions($campaign_id, $subscriber_id = NULL, $action = NULL, $cache = true){
		...
		if(false === ($actions = mymail_cache_get( 'campaign_actions' ))){
			...
			if($cache) mymail_cache_add( 'campaign_actions', $actions );
