...
class RS_CSV_Importer extends WP_Importer {
	...
	*/
	...
	// Step 1
	function greet() {
		echo '<p>'.__( 'Scegli un file CSV (.csv) da caricare, quindi fai clic su Carica file e importa.', 'really-simple-csv-importer' ).'</p>';
		...
		echo '<p>'.__( 'Requisiti:', 'really-simple-csv-importer' ).'</p>';
		echo '<ol>';
		echo '<li>'.__( 'Selezionare UTF-8 come carattere.', 'really-simple-csv-importer' ).'</li>';
		echo '<li>'.sprintf( __( 'È necessario utilizzare il seguente delimitatore di campo "%s"', 'really-simple-csv-importer'), RS_CSV_Helper::DELIMITER ).'</li>';
		//echo '<li>'.__( 'You must quote all text cells.', 'really-simple-csv-importer' ).'</li>';
		echo '</ol>';
		echo '<p>'.__( 'Scarica esempio file CSV:', 'really-simple-csv-importer' );
	...
	function import() {
		...
		if ( isset( $file['error'] ) ) {
			echo '<p><strong>' . __( 'Errore', 'really-simple-csv-importer' ) . '</strong><br />';
	...
	function process_posts() {
		...
		while (($data = $h->fgetcsv($handle)) !== FALSE) {
			if ($is_first) {
				...
				// (int) post id
				$post_id = $h->get_data($this,$data,'ID CALENDARIO');
				...
				$comment_address = $h->get_data($this,$data,'address');
				if ($comment_address) {
					$post['address'] = $comment_address;
				}

				// (int) post date
				$post_date = $h->get_data($this,$data,'DATA');
				if ($post_date) {
					$post['DATA'] = $post_date;
					$myDateTime = DateTime::createFromFormat('d/m/y', $post['DATA']);
					$newDateString = $myDateTime->format('Ymd');
					$post_date = $newDateString;
				}

				$post_address = $h->get_data($this,$data,'INDIRIZZO');
				if ($post_address) {
					$post['INDIRIZZO'] = $post_address;
				}
				...
				$post_activity = $h->get_data($this,$data,'ID ATTIVITà');
				if ($post_activity) {
					$post['ID ATTIVITà'] = $post_activity;
				}
				...
				// (string, comma separated) name of post tags
				$post_city = $h->get_data($this,$data,'LUOGO');
				if ($post_city) {
					$post['LUOGO'] = $post_city;



					//$i = add_row('locations_copia', $row);
					//update_field('locations_copia_0_city', $row, 68);
					//echo "string";


				$rows = get_field('locations',$post_id);

				$i = 0;
					if ($rows != null && isset($rows) && count($rows) > 0) {
						$i = count($rows);
					}
					$row1 = array(
							'locations_' . $i . '_city'	=> $post_city,
							'locations_' . $i . '_date'	=> $post_date,
							'locations_' . $i . '_activity'	=> $post_activity,
							'locations_' . $i . '_address'	=> $post_address,

						);

					$add_row = add_row('locations', $row1,$post_id);
					$add_row = add_row('locations_' . $i . '_city', $row1['locations_' . $i . '_city'],$post_id);
					$add_row = add_row('locations_' . $i . '_date', $row1['locations_' . $i . '_date'],$post_id);
					$add_row = add_row('locations_' . $i . '_activity', $row1['locations_' . $i . '_activity'],$post_id);
					$add_row = add_row('locations_' . $i . '_address', $row1['locations_' . $i . '_address'],$post_id);

					update_field('locations_' . $i . '_city', $row1['locations_' . $i . '_city'],$post_id);
					update_field('locations_' . $i . '_date', $row1['locations_' . $i . '_date'],$post_id);
					update_field('locations_' . $i . '_activity', $row1['locations_' . $i . '_activity'],$post_id);
					update_field('locations_' . $i . '_address', $row1['locations_' . $i . '_address'],$post_id);
				...
				if (!$error->get_error_codes() && $dry_run == false) {
					...
					if ($result->isError()) {
						...
						echo esc_html(sprintf(__('Importazione "%s" effettuata.', 'really-simple-csv-importer'), $post_title));
					}
				}
		...
		echo '<h3>'.__('Completato.', 'really-simple-csv-importer').'</h3>';
	}
