...
import { CatalogBrandRenderComponent } from './catalog-brand-render.component';

@Component({
  selector: 'ngx-catalog-items',
...
export class CatalogItemsComponent {
  ...
  settings = {
    ...
    columns: {
      ...
      brands: {
        ...
        sort: false,
        type: 'custom',
        renderComponent: CatalogBrandRenderComponent,
        ...
        //       brands += element.name + ' - (' + element.id + ')';
        //     });
        //     return brands;
        //   }
        // }
  ...
  constructor(private http: HttpClient,
    ...
    this.source = new CustomServerDataSource(http, {
      endPoint: environment.endpoint+'/v1/catalogs/'+this.id+'/items?size=true',
      pagerLimitKey:"limit",
      pagerPageKey:"page",
      sortDirKey: "order",
      sortFieldKey: "sort",
  ...
  onEdit(event): void {
    ...
    this.router.navigate(['pages/catalogs', this.id, 'items', event.data.id]).then( (e) => {
      if (e) {
  ...
  //     this.catalogsService.delete(event.data.id, '').subscribe((res: StgdrvResponseDTO) => {
  //       console.log('Res', res)
  //       this.source.refresh();
  //       this.loading = false;
